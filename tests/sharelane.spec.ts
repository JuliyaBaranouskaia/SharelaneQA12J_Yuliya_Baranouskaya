...
describe("Test Suite for Sharelane", async () => {
  ...
  it("New user registration - invalid email", async function () {
    ...
    const confirmationMessage = await driver
      .findElement(By.css(".error_message"))
      .getText();
    expect(confirmationMessage).to.include(
  ...
  it("User login - valid data", async function () {
    ...
    const emailElement = await driver.findElement(
      By.xpath("//td[text()='Email']/following-sibling::td/b"),
    );
    const passwordElement = await driver.findElement(
      By.xpath("//td[text()='Password']/following-sibling::td"),
    );
    ...
    const logoutElement = await driver.findElement(
      By.css("tr.grey_bg a[href='./log_out.py']"),
    );
